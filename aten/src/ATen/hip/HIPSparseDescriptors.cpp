...
hipsparseStatus_t destroyConstDnMat(const cusparseDnMatDescr* dnMatDescr) {
  ...
  return hipsparseDestroyDnMat(const_cast<cusparseDnMatDescr*>(dnMatDescr));
}

namespace {

// If a specific GPU model does not provide native support for a given data
// type, cuSparse routines return HIPSPARSE_STATUS_ARCH_MISMATCH error
void check_supported_cuda_type(hipDataType cuda_type) {
  if (cuda_type == HIP_R_16F) {
...
hipsparseDnMatDescr_t createRawDnMatDescriptor(const Tensor& input, int64_t batch_offset, bool is_const=false) {
  ...
  hipDataType value_type = ScalarTypeToCudaDataType(input.scalar_type());
  ...
  hipsparseDnMatDescr_t raw_descriptor = nullptr;
  ...
  if (ndim >= 3 && batch_offset == -1) {
    ...
    TORCH_CUDASPARSE_CHECK(hipsparseDnMatSetStridedBatch(
...
CuSparseDnVecDescriptor::CuSparseDnVecDescriptor(const Tensor& input) {
  ...
  hipsparseDnVecDescr_t raw_descriptor = nullptr;
  TORCH_CUDASPARSE_CHECK(hipsparseCreateDnVec(
...
CuSparseSpMatCsrDescriptor::CuSparseSpMatCsrDescriptor(const Tensor& input, int64_t batch_offset) {
  ...
  hipsparseSpMatDescr_t raw_descriptor = nullptr;
  ...
  if (ndim == 3 && batch_offset == -1) {
    ...
    if (crow_indices.dim() >= 2 || values.dim() >= 2 ||
      ...
      TORCH_CUDASPARSE_CHECK(hipsparseCsrSetStridedBatch(
          raw_descriptor,
          batch_count,
          crow_indices.stride(-2),
          values_->stride(-2)));
      ...
      TORCH_CUDASPARSE_CHECK(
          hipsparseCsrSetStridedBatch(raw_descriptor, batch_count, 0, 0));
    }
  }
