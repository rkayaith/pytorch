...
template <>
std::function<bool(hipStream_t)> CUDAGraph::create_allocate_filter<hipStream_t>() const {
  return [this](hipStream_t stream) {
...
std::function<bool(c10::Stream)> CUDAGraph::create_allocate_filter<c10::Stream>() const {
  return [this](c10::Stream stream) {
    hipStream_t cuda_stream = CUDAStream(CUDAStream::UNCHECKED, stream);
    auto capture_id_opt = c10::cuda::captureIdMayInitCtx(cuda_stream);
    return capture_id_opt.has_value() && capture_id_opt.value() == capture_id_;
  };
}

void CUDAGraph::capture_begin(MempoolId_t pool/*={0,0}*/, hipStreamCaptureMode capture_mode) {
  ...
  c10::cuda::CUDACachingAllocator::beginAllocateToPool(capture_dev_, mempool_id_, create_allocate_filter<hipStream_t>());
  ...
  TORCH_INTERNAL_ASSERT(capture_id_opt.has_value(),
      "Stream should be actively capturing after hipStreamBeginCapture");
...
void CUDAGraph::capture_end() {
  ...
  // Capture is over once hipStreamEndCapture returns (success or failure).
  ...
  hipError_t endCaptureErr = hipStreamEndCapture(capture_stream_, &graph_);
  {
    std::unique_lock<std::mutex> lock(_currently_capturing_graphs_mutex);
    TORCH_CHECK(
...
void CUDAGraph::replay() {
  ...
  AT_CUDA_CHECK(hipGraphLaunch(graph_exec_, at::cuda::getCurrentCUDAStream()));
}
...
void CUDAGraph::debug_dump(const std::string& debug_path) {
  if (_cuda_graphs_debug || keep_graph_) {
    ...
    if (has_graph_) {
      ...
      C10_CUDA_CHECK_WARN(hipGraphDebugDotPrint(graph_, debug_path.c_str(), hipGraphDebugDotFlagsVerbose)); // most verbose output
      if (!keep_graph_) {
        AT_CUDA_CHECK(hipGraphDestroy(graph_));
        has_graph_ = false;
      }
...
void CUDAGraph::reset() {
  ...
  if (has_graph_) {
    C10_CUDA_CHECK_WARN(hipGraphDestroy(graph_));
    has_graph_ = false;
  }
  if (has_graph_exec_) {
    C10_CUDA_CHECK_WARN(hipGraphExecDestroy(graph_exec_));
...
void CUDAGraph::begin_capture_to_if_node(
...
#if !defined(USE_ROCM) && (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12040)
  TORCH_CHECK(
      !has_graph_exec_,
      "This CUDAGraph instance already owns a captured graph.");
  ...
  hipGraph_t currently_capturing_graph{};
  ...
  const hipGraphEdgeData* dependency_edges{};
  size_t num_dependencies = 0;
#if TORCH_HIP_VERSION >= 13000
  AT_CUDA_CHECK(hipStreamGetCaptureInfo(
...
#endif
  TORCH_CHECK(status == hipStreamCaptureStatusActive);

  hipGraphNodeParams params{};
  params.type = hipGraphNodeTypeConditional;
  ...
  AT_CUDA_CHECK(cudaGraphAddNode_v2(
      ...
      &params));
#endif
  hipGraph_t if_node_child_graph = params.conditional.phGraph_out[0];

#if TORCH_HIP_VERSION >= 13000
  AT_CUDA_CHECK(hipStreamUpdateCaptureDependencies(
getCurrentCUDAStream(), &cond_node, nullptr, 1, hipStreamSetCaptureDependencies));
#else
...
#else // !defined(USE_ROCM) && (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12040)
  AT_ERROR(
      __func__,
...
void CUDAGraph::end_capture_to_conditional_node() {
  ...
  AT_CUDA_CHECK(hipStreamEndCapture(stream.stream(), nullptr));
  ...
  if (conditional_graph_capture_ids_.empty()) {
    c10::cuda::CUDACachingAllocator::beginAllocateToPool(
        capture_dev_, mempool_id_, create_allocate_filter<hipStream_t>());
    at::getHostAllocator(at::kCUDA)->begin_allocate_to_pool(mempool_id_, create_allocate_filter<c10::Stream>());
  } else {
    c10::cuda::CUDACachingAllocator::beginAllocateToPool(
        capture_dev_, mempool_id_, create_child_allocate_filter());
...
std::function<bool(hipStream_t)> CUDAGraph::create_child_allocate_filter() {
#if !defined(USE_ROCM) && (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12040)
  return [&current_capture_id = conditional_graph_capture_ids_.top()](hipStream_t stream) {
      auto capture_id_opt = c10::cuda::captureIdMayInitCtx(stream);
  ...
  AT_ERROR(
      __func__,
      " CUDA Graphs conditional nodes are not supported for cuda version < 12.4");
  return std::function<bool(hipStream_t)>();
#endif
}
