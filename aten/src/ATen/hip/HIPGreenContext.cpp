...
#if defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12080 && !defined(USE_ROCM) && defined(PYTORCH_C10_DRIVER_API_SUPPORTED)
...
#if defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13010 && HAS_CUDA_GREEN_CONTEXT()
...
GreenContext::GreenContext(
  ...
  int driver_version;
  C10_CUDA_CHECK(hipDriverGetVersion(&driver_version));
  TORCH_CHECK(
...
uint32_t GreenContext::max_workqueue_concurrency(
  ...
  CUDAStream GreenContext::Stream() {
    ...
    if (curr_stream_idx_ < kStreamPerGreenContextPool) {
       hipStream_t green_ctx_side_stream;
       C10_CUDA_DRIVER_CHECK(c10::cuda::DriverAPI::get()->cuGreenCtxStreamCreate_(
         &green_ctx_side_stream, green_ctx_, hipStreamNonBlocking, 0));
