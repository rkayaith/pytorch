...
#define AT_CUDA_DRIVER_CHECK(EXPR)                                          \
  do {                                                                      \
    ...
    if (__err != hipSuccess) {                                            \
      const char* err_str;                                                  \
      [[maybe_unused]] hipError_t get_error_str_err =                         \
          at::globalContext().getNVRTC().hipDrvGetErrorString(__err, &err_str); \
  ...
  do {                                                                                              \
    ...
    if (__err != HIPRTC_SUCCESS) {                                                                   \
      if (static_cast<int>(__err) != 7) {                                                           \
        TORCH_CHECK(false, "CUDA NVRTC error: ", at::globalContext().getNVRTC().hiprtcGetErrorString(__err));  \
      } else {                                                                                      \
        TORCH_CHECK(false, "CUDA NVRTC error: HIPRTC_ERROR_BUILTIN_OPERATION_FAILURE");                        \
      }                                                                                             \
    }                                                                                               \
  } while (0)
