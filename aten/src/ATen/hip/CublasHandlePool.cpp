...
void clearCublasWorkspacesForStream(hipStream_t stream) {
...
void setWorkspaceForHandle(hipblasHandle_t handle, c10::cuda::CUDAStream stream) {
...
#ifndef USE_ROCM
  if (unified_cublas_and_lt_workspaces()) {
    hipblasHandle_t handle = at::cuda::getCurrentCUDABlasHandle(/*setup=*/false);
    ...
    // No need to call hipblasSetWorkspace; Lt passes workspace explicitly.
    auto new_workspace = getNewWorkspace();
    {
      std::unique_lock<std::shared_mutex> lock(workspace.mutex);
      auto workspace_it = workspace.map.try_emplace(key, std::move(new_workspace)).first;
...
hipblasHandle_t getCurrentCUDABlasHandle(bool setup) {
...
hipblasLtHandle_t getCurrentCUDABlasLtHandle() {
  ...
  return handle;
#else
  return reinterpret_cast<hipblasLtHandle_t>(getCurrentCUDABlasHandle(/*setup=*/false));
#endif
}
