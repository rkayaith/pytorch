...
struct MAGMAQueue {
  ...
  explicit MAGMAQueue(int64_t device_id) {
    ...
    TORCH_CUDABLAS_CHECK(hipblasGetMathMode(handle, &original_math_mode));
  ...
  ~MAGMAQueue() {
    ...
    hipblasHandle_t handle = magma_queue_get_cublas_handle(magma_queue_);
    hipblasSetMathMode(handle, original_math_mode);
#endif
