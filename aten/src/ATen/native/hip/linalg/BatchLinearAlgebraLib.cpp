...
static hipblasOperation_t to_cublas(TransposeType trans) {
  switch (trans) {
    case TransposeType::NoTranspose: return HIPBLAS_OP_N;
    case TransposeType::Transpose: return HIPBLAS_OP_T;
    case TransposeType::ConjTranspose: return HIPBLAS_OP_C;
  }
...
static void apply_svd_cusolver_gesvdj(const Tensor& A, const Tensor& U, const Tensor& S, const Tensor& V,
  ...
  for(int i = 0; i < batchsize; i++){
    ...
    // double residual = 0;
    // TORCH_CUSOLVER_CHECK(hipsolverDnXgesvdjGetResidual(handle, gesvdj_params, &residual));
...
static void apply_geqrf(const Tensor& A, const Tensor& tau) {
  ...
  hipsolverDnParams_t params = nullptr; // use default algorithm (currently it's the only option)
