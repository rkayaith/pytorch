...
C10_CUDA_API hipError_t GetDeviceCount(int* dev_count);

C10_CUDA_API hipError_t GetDevice(DeviceIndex* device);

C10_CUDA_API hipError_t
SetDevice(DeviceIndex device, const bool force = false);

C10_CUDA_API hipError_t MaybeSetDevice(DeviceIndex device);
...
#else
  C10_CUDA_CHECK(hipMemcpyAsync(dst, src, nbytes, kind, stream));
  C10_CUDA_CHECK(hipStreamSynchronize(stream));
#endif
}

C10_CUDA_API void __inline__ stream_synchronize(hipStream_t stream) {
  if (C10_UNLIKELY(
          warning_state().get_sync_debug_mode() != SyncDebugMode::L_DISABLED)) {
    warn_or_error_on_sync();
  }
