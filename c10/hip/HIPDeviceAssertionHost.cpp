...
int dsa_get_device_compute_capability(const int device_num) {
  ...
  C10_CUDA_CHECK_WO_DSA(hipDeviceGetAttribute(
...
void uvm_deleter(DeviceAssertionsData* uvm_assertions_ptr) {
  ...
  if (uvm_assertions_ptr) {
    C10_CUDA_IGNORE_ERROR(hipFree(uvm_assertions_ptr));
  }
}
...
DeviceAssertionsData* CUDAKernelLaunchRegistry::
  ...
  hipMemLocation cpuDevice;
  cpuDevice.type = hipMemLocationTypeDevice;
  cpuDevice.id = hipCpuDeviceId;
#ifdef USE_ROCM
  // hipify replaces hipMemAdvise -> hipMemAdvise, but we want v2
#define hipMemAdvise hipMemAdvise_v2
  ...
  C10_CUDA_CHECK_WO_DSA(hipMemAdvise(
      uvm_assertions_ptr,
