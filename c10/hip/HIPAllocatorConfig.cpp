...
#define PYTORCH_TOKEN2 "hipMallocAsync"
  ...
  if (used_cudaMallocAsync) {
    ...
    int version = 0;
    C10_CUDA_CHECK(hipDriverGetVersion(&version));
    TORCH_CHECK(
        version >= 11040,
        "backend:hipMallocAsync requires CUDA runtime "
    ...
    TORCH_CHECK(
        ...
        "CUDA 11.4 or newer, but TORCH_HIP_VERSION is ",
        TORCH_HIP_VERSION);
...
size_t CUDAAllocatorConfig::parseThrowOnCudaMallocOom(
  ...
  // When enabled, throws OOM error before calling hipMalloc if the allocation
