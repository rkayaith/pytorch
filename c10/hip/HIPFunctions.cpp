...
int32_t driver_version() {
  ...
  C10_CUDA_IGNORE_ERROR(hipDriverGetVersion(&driver_version));
...
// of `hipSetDevice`. Unlike to previous CUDA versions that allocate context
// lazily CUDA 12.x eagerly allocates primary context the moment `hipSetDevice`
...
hipError_t SetDevice(DeviceIndex device, const bool force) {
  ...
  int cur_device = -1;
  C10_CUDA_CHECK(hipGetDevice(&cur_device));
...
DeviceIndex ExchangeDevice(DeviceIndex to_device) {
  ...
  if (cur_device < 0) {
    int tmp_device = -1;
    C10_CUDA_CHECK(hipGetDevice(&tmp_device));
    cur_device = static_cast<DeviceIndex>(tmp_device);
    if (to_device == cur_device) {
      return cur_device;
    }
  }
  C10_CUDA_CHECK(hipSetDevice(to_device));
  return cur_device;
}
...
DeviceIndex MaybeExchangeDevice(DeviceIndex to_device) {
  ...
  C10_CUDA_CHECK(hipGetDevice(&tmp_cur_device));
  TORCH_INTERNAL_ASSERT(
      tmp_cur_device >= 0 &&
