...
struct CUDAGuardImpl final : public c10::impl::DeviceGuardImplInterface {
  ...
  void createEvent(hipEvent_t* cuda_event, const EventFlag flag) const {
    // Maps PyTorch's Event::Flag to CUDA flag
    auto cuda_flag = hipEventDefault;
    switch (flag) {
      case EventFlag::PYTORCH_DEFAULT:
        cuda_flag = hipEventDisableTiming;
        break;
      case EventFlag::BACKEND_DEFAULT:
        cuda_flag = hipEventDefault;
        break;
      default:
        TORCH_CHECK(false, "CUDA event received unknown flag");
    }

    C10_CUDA_CHECK(hipEventCreateWithFlags(cuda_event, cuda_flag));
  ...
  void destroyEvent(void* event, const DeviceIndex device_index)
    ...
    if (!event)
      return;
    auto cuda_event = static_cast<hipEvent_t>(event);
    ...
    C10_CUDA_CHECK_WARN(hipEventDestroy(cuda_event));
  ...
  void record(
    ...
    C10_CUDA_CHECK(hipEventRecord(cuda_event, cuda_stream));
  ...
  void block(void* event, const Stream& stream) const override {
    ...
    const auto orig_device = getDevice();
    setDevice(stream.device());
    C10_CUDA_CHECK(hipStreamWaitEvent(
        cuda_stream,
        cuda_event,
  ...
  bool queryEvent(void* event) const override {
    ...
    const hipError_t err = C10_CUDA_ERROR_HANDLED(hipEventQuery(cuda_event));
  ...
  void synchronizeEvent(void* event) const override {
    ...
    C10_CUDA_CHECK(hipEventSynchronize(cuda_event));
  }
  ...
  double elapsedTime(void* event1, void* event2, const DeviceIndex device_index)
    ...
    C10_CUDA_CHECK(c10::cuda::GetDevice(&orig_device));
    C10_CUDA_CHECK(c10::cuda::SetDevice(device_index));
    hipEvent_t cuda_event1 = static_cast<hipEvent_t>(event1);
    hipEvent_t cuda_event2 = static_cast<hipEvent_t>(event2);
    float time_ms = 0;
    // raise hipErrorNotReady if either event is recorded but not yet completed
    C10_CUDA_CHECK(hipEventElapsedTime(&time_ms, cuda_event1, cuda_event2));
    C10_CUDA_CHECK(c10::cuda::SetDevice(orig_device));
