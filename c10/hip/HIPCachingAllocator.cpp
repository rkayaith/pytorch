...
#include <c10/hip/PeerToPeerAccess.h>
...
struct ExpandableSegment {
  ExpandableSegment(
    ...
    C10_CUDA_CHECK(hipGetDeviceProperties(&prop, device_));
    mapped_size_ = 0;
...
#define SYS_pidfd_getfd 438
 ...
 private:
  void setAccess(c10::DeviceIndex device, size_t begin, size_t end) {
    ...
    hipMemAccessDesc desc[num_desc];
  ...
  void unmapHandles(size_t begin, size_t end) {
    ...
    if (stream_) {
      C10_CUDA_CHECK(hipStreamSynchronize(*stream_));
    } else {
...
hipError_t allocPrimitive(void** ptr, size_t size, AllocParams& p) {
  if (p.pool->owner_PrivatePool && p.pool->owner_PrivatePool->allocator()) {
    *ptr = p.pool->owner_PrivatePool->allocator()->raw_alloc(size);
    ...
    return C10_CUDA_ERROR_HANDLED(hipMalloc(ptr, size));
  }
}

hipError_t cudaMallocMaybeCapturing(void** ptr, size_t size, AllocParams& p) {
  if (at::cuda::currentStreamCaptureStatusMayInitCtx() ==
      at::cuda::CaptureStatus::None) {
    return allocPrimitive(ptr, size, p);
  } else {
    // It's ok to capture cudaMallocs, as long as we never hipFree those
    // addresses before replay.
    // Capturing hipMalloc behaves nicely: it gives the graph new VA,
    // but is ignored (won't leakily allocate new memory) in replays.
    at::cuda::CUDAStreamCaptureModeGuard g{hipStreamCaptureModeRelaxed};
    return allocPrimitive(ptr, size, p);
  }
}
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  explicit DeviceCachingAllocator(c10::DeviceIndex id)
    ...
    C10_CUDA_CHECK(hipGetDeviceProperties(&device_prop, id));
...
#if (defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13000)
      if (deps == nullptr) {
        C10_CUDA_CHECK(hipGraphNodeGetDependencies(n, deps, nullptr, count));
      } else {
...
#else
      C10_CUDA_CHECK(hipGraphNodeGetDependencies(n, deps, count));
#endif
    };
  ...
  ShareableHandle shareIpcHandle(Block* block) {
    ...
    if (!block->expandable_segment_) {
      ...
      C10_CUDA_CHECK(hipIpcGetMemHandle(&handle, base_block->ptr));
  ...
  void endAllocateToPool(MempoolId_t mempool_id) {
    ...
    if (CUDAAllocatorConfig::graph_capture_record_stream_reuse()) {
      ...
      // hipStreamEndCapture would have failed if any stream used during
 ...
 private:
  ...
  void release_block(
    ...
    if (pool->owner_PrivatePool && pool->owner_PrivatePool->allocator()) {
      ...
      C10_CUDA_CHECK(hipFree((void*)block->ptr));
    }
  ...
  void synchronize_and_free_events(
    ...
    for (auto it = cuda_events.begin(); it != cuda_events.end();) {
      for (auto e = it->second.begin(); e != it->second.end();) {
        ...
        C10_CUDA_CHECK(hipEventSynchronize(*event));
  ...
  void insert_events(Block* block) {
    ...
    for (auto& stream : streams) {
      ...
      C10_CUDA_CHECK(hipEventRecord(*event, stream.stream()));
  ...
  void process_events(const std::shared_ptr<GatheredContext>& context) {
    ...
    for (auto it = cuda_events.begin(); it != cuda_events.end();) {
      ...
      while (!it->second.empty()) {
        ...
        hipError_t err = C10_CUDA_ERROR_HANDLED(hipEventQuery(*event));
...
static void* uncached_allocate(size_t size) {
  ...
  C10_CUDA_CHECK(hipMalloc(&devPtr, size));
  ...
  struct MemHandleCacheEntry {
    MemHandleCacheEntry(
      ...
      if (type == SHAREABLE_CUDA_MALLOC) {
        ...
        ss.read(reinterpret_cast<char*>(&cuda_handle), HIP_IPC_HANDLE_SIZE);
        C10_CUDA_CHECK(hipIpcOpenMemHandle(
            &cuda_ipc_ptr_, cuda_handle, hipIpcMemLazyEnablePeerAccess));
    ...
    void clear() {
      if (cuda_ipc_ptr_) {
        cuda::CUDAGuard device_guard(device_);
        C10_CUDA_CHECK(hipIpcCloseMemHandle(cuda_ipc_ptr_));
