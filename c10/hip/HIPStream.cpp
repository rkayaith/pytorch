...
void initSingleStream(int p, DeviceIndex device_index, int i) {
  ...
  C10_CUDA_CHECK(hipStreamCreateWithPriority(&stream, kDefaultFlags, pri));
...
bool CUDAStream::query() const {
  ...
  hipError_t err = C10_CUDA_ERROR_HANDLED(hipStreamQuery(stream()));
...
hipStream_t CUDAStream::stream() const {
