...
inline void lazy_init_device(c10::DeviceIndex device) {
  if (!devs_initialized_flags[device]) {
    ...
    C10_CUDA_CHECK(hipDeviceGetDefaultMemPool(&mempool, device));
    uint64_t threshold = UINT64_MAX;
    C10_CUDA_CHECK(hipMemPoolSetAttribute(
...
inline void sync_raw(hipStream_t dependency, hipStream_t dependent) {
  ...
  C10_CUDA_CHECK(hipEventCreateWithFlags(&event, hipEventDisableTiming));
  C10_CUDA_CHECK(hipEventRecord(event, dependency));
  C10_CUDA_CHECK(hipStreamWaitEvent(dependent, event));
  C10_CUDA_CHECK(hipEventDestroy(event));
}
...
inline void free_impl(PtrInfo::iterator& it) {
  ...
  if (recorded_streams.empty()) {
    ...
    C10_CUDA_CHECK(hipFreeAsync(it->first, creation_stream.stream));
    ...
    C10_CUDA_CHECK(hipFreeAsync(it->first, dummy_unifying_free_stream.stream));
...
struct CudaMallocAsyncAllocator : public CUDAAllocator {
  ...
  void emptyCache(/*unused*/ MempoolId_t mempool_id) override {
    ...
    for (int dev = 0; dev < device_count; dev++) {
      if (devs_initialized_flags[dev]) {
        ...
        hipMemPool_t mempool = nullptr;
        C10_CUDA_CHECK(hipDeviceGetDefaultMemPool(&mempool, dev));
        C10_CUDA_CHECK(hipDeviceSynchronize());
        C10_CUDA_CHECK(hipMemPoolTrimTo(mempool, 0));
      }
    }
  }
  ...
  void cacheInfo(c10::DeviceIndex device, size_t* maxWorkspaceGuess) override {
    ...
    size_t device_total = 0;
    C10_CUDA_CHECK(hipMemGetInfo(&free_upper_bound, &device_total));
    TORCH_INTERNAL_ASSERT(
    ...
    while (true) {
      ...
      if (err == hipSuccess) {
        C10_CUDA_CHECK(hipFreeAsync(dummy, stream));
  ...
  void attachOomRejectionObserver(OomRejectionObserver observer) override {
    TORCH_CHECK(
        false,
        "hipMallocAsync does not yet support attachOomRejectionObserver. "
        "If you need it, please file an issue describing your use case.");
  }
  ...
  DeviceStats getDeviceStats(c10::DeviceIndex device) override {
    ...
    if (devs_initialized_flags[device]) {
      ...
      hipMemPool_t mempool = nullptr;
      C10_CUDA_CHECK(hipDeviceGetDefaultMemPool(&mempool, device));
      C10_CUDA_CHECK(hipMemPoolGetAttribute(
  ...
  void endAllocateToPool(c10::DeviceIndex device, MempoolId_t mempool_id)
    ...
    for (const auto& free_stream : capture_free_streams) {
      ...
      hipEvent_t event = nullptr;
      C10_CUDA_CHECK(hipEventCreateWithFlags(&event, hipEventDisableTiming));
      C10_CUDA_CHECK(hipEventRecord(event, free_stream.stream));
      C10_CUDA_CHECK(hipStreamWaitEvent(capture_stream.stream(), event));
      C10_CUDA_CHECK(hipEventDestroy(event));
    }
  ...
  void enablePeerAccess(c10::DeviceIndex dev, c10::DeviceIndex dev_to_access)
    ...
    hipMemPool_t mempool = nullptr;
    C10_CUDA_CHECK(hipDeviceGetDefaultMemPool(&mempool, dev_to_access));
    hipMemAccessDesc desc = {};
    desc.location.type = hipMemLocationTypeDevice;
    // NOLINTNEXTLINE(bugprone-signed-char-misuse)
    desc.location.id = dev;
    desc.flags = hipMemAccessFlagsProtReadWrite;
    C10_CUDA_CHECK(hipMemPoolSetAccess(mempool, &desc, 1 /* numDescs */));
  }
